...
def find_3x3_spaces(grid: Grid) -> None:
    """
    Cherche et remplit les espaces vides 3x3 avec deux motifs possibles:
    - Pattern1: sous forme de 91 avec LINE et CORNER
    - Pattern2: CROSS central avec T_SHAPE
    """

    def is_empty_3x3(x: int, y: int) -> bool:
        if x + 2 >= len(grid[0]) or y + 2 >= len(grid):
            return False
        return all(
            grid[y + i][x + j] == [0, 0, 0, 0]
            for i in range(3)
            for j in range(3)
        )

    def pattern1(x: int, y: int):
        patterns = [
        ...
        for i in range(3):
            for j in range(3):
                grid[y + i][x + j] = patterns[i][j]

    def pattern2(x: int, y: int):
        patterns = [
